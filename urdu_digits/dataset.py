...
class UrduDigitDataset(Dataset):
    ...
    def __init__(self, root_dir, csv_file, transform=None, is_test=False):
        ...
        if not is_test:
            # Training images are stored in subfolders named after their digit
            for _, row in self.df.iterrows():
                img_id = row['Id']
                label = row['Category']
                # Path format based on exploration: data/train/train/label/id.png
                path = os.path.join(self.root_dir, str(label), f"{int(img_id)}.png")
                self.image_paths.append(path)
                self.labels.append(label)
        else:
            # Test images are stored in a single flat directory
            for _, row in self.df.iterrows():
                img_id = row['Id']
    ...
    def __getitem__(self, idx):
        ...
        if self.is_test:
            # Return image and its ID for submission mapping
